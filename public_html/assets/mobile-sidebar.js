  ...
  async function renderAnnualReport(requestedYear) {
    ...
    var load = async function (selectedYear) {
      var rows = await Promise.all(Array.from({ length: 12 }, function (_, index) {
        ...
        var key = selectedYear + '-' + month;
        return fetch('/api/finance.php?period=' + key, { cache: 'no-store' }).then(function (response) { return response.ok ? response.json() : null; }).catch(function () { return null; }).then(function (item) { return item && item.period && item.period.period_key === key ? item : { period: { period_key: key, label: new Date(Number(selectedYear), index, 1).toLocaleString('en-US', { month: 'long', year: 'numeric' }) }, summary: { salesCentavos: 0, purchasesCentavos: 0, expensesCentavos: 0, netChangeCentavos: 0 } }; });
      }));
