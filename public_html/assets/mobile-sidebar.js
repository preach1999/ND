(function () {
  ...
  async function renderAnnualReport(requestedYear) {
    ...
    var load = async function (selectedYear) {
      ...
      if (first && last) main.querySelector('.annual-report-grid').insertAdjacentHTML('beforeend', '<section class="annual-profit-card annual-cash-card"><h2>Cash position</h2><div class="annual-summary-row"><span>Opening balance</span><strong>' + money(first.summary.openingBalanceCentavos || 0) + '</strong></div><div class="annual-summary-row"><span>Net cash change</span><strong class="annual-positive">+' + money(totals.net) + '</strong></div><div class="annual-summary-total"><span>Current balance</span><strong>' + money(last.summary.currentBalanceCentavos || 0) + '</strong></div></section>');
